...
def lancement_export_ouvrage(url_general, liste_ouvrage_categorie):
    soup_actuel=export_ouvrage(url_general,liste_ouvrage_categorie)#premier passage
    while soup_actuel.find("ul",class_="pager"): # Vérifie que le bandeau de changement de page existe
        ...
        if soup_provisoire.find("li", class_="next"):# Vérifie qu'il existe une page suivante
            ...
            url_actuel_categorie=urljoin(str(url_general), str(url_provisoire)) # Fusionne les deux URL pour avoir l'URL complet
...
#------- Récupére le fichier image (nom+fichier)
    ...
    adresse_image=fichier_image+"/"+balise_category+"/"+nom_livre[-2]+".jpg"
    if not os.path.exists(adresse_image):# Si la photo n'existe pas, on l'enregistre
        f = open(adresse_image,'wb')#Enregistrement du fichier image
